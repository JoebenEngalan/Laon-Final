...
window.onload = function() {
    ...
    objectStore.openCursor().onsuccess = function(event) {
      let cursor = event.target.result;
        ...
        if(cursor) {
          ...
          listItem.innerHTML =  '<br>'+ 'Reading: '+ cursor.value.taskTitle + ' <br>' + cursor.value.hours + ':' + cursor.value.minutes + ', ' + cursor.value.month + ' ' + cursor.value.day + daySuffix + ' ' + cursor.value.year + '.' + '&nbsp;'+ '&nbsp;' + '&nbsp;';
  ...
  function createNotification(title) {
    ...
    let img = '/LAON/imgages/LAON-128.png';
